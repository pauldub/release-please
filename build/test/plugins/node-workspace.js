...
const prettyJsonStringify = (value, replacer) => JSON.stringify(value, replacer, 2) + '\n';
...
function stringifyActual(actual) {
    ...
    for (const pkgsWithPRData of actual) {
        ...
        stringified +=
            '='.repeat(20) +
                '\n' +
                prettyJsonStringify(pkgsWithPRData, (k, v) => k === ' changes' ? undefined : v);
...
mocha_1.describe('NodeWorkspaceDependencyUpdates', () => {
    mocha_1.afterEach(() => {
        sandbox.restore();
    });
    ...
    mocha_1.describe('run', () => {
        mocha_1.it('handles a simple chain where root pkg update cascades to dependents', async function () {
            ...
            const nodeWS = new node_workspace_1.default(github, config, 'node-workspace', checkpoint);
            const [actualManifest, actualChanges] = await nodeWS.run(newManifestVersions, pkgsWithPRData);
            mock.verify();
        ...
        mocha_1.it('prefers release-as configuration over default patch-bump', async function () {
        ...
        mocha_1.it('does not update dependencies on preMajor versions with minor bump', async function () {
            const config = {
                packages: {},
                parsedPackages: [
                    { path: 'packages/pkgA', releaseType: 'node' },
        ...
        mocha_1.it('handles unusual changelog formats', async function () {
            const config = {
                packages: {},
                parsedPackages: [
                    { path: 'packages/pkgA', releaseType: 'node' },
        ...
        mocha_1.it('handles errors retrieving changelogs', async function () {
            const config = {
                packages: {},
                parsedPackages: [
                    { path: 'packages/pkgA', releaseType: 'node' },
        ...
        mocha_1.it('updates dependent from pre-release version', async function () {
            const config = {
                packages: {},
                parsedPackages: [
                    { path: 'packages/pkgA', releaseType: 'node' },
        ...
        mocha_1.it('does not update dependency to pre-release version', async function () {
            const config = {
                packages: {},
                parsedPackages: [
                    { path: 'packages/pkgA', releaseType: 'node' },
            ...
            const [actualManifest, actualChanges] = await nodeWS.run(newManifestVersions, pkgsWithPRData);
            mock.verify();
            chai_1.expect([...actualManifest]).to.eql([['packages/pkgA', '1.1.2-alpha.0']]);
            const snapPrefix = this.test.fullTitle();
            snapshot(snapPrefix + ' logs', logs);
            snapshot(snapPrefix + ' changes', stringifyActual(actualChanges));
        });
        mocha_1.it('does not update dependent with invalid version', async function () {
            const github = new github_1.GitHub({
                owner: 'fake',
                repo: 'repo',
                defaultBranch: 'main',
