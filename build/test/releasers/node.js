...
const package_lock_json_1 = require("../../src/updaters/package-lock-json");
    ...
    if (fixtures.length !== 0) {
        for (const fixture of fixtures) {
            mock
                ...
                .once()
                .resolves(utils_1.buildGitHubFileContent('./test/releasers/fixtures/node', path_1.basename(fixture)));
        }
    }
...
mocha_1.describe('Node', () => {
    ...
    mocha_1.describe('getOpenPROptions', () => {
        ...
        testDefaultInitialVersion.forEach(element => {
            mocha_1.it(`returns release PR changes with defaultInitialVersion ${element.version}, ` +
                `when bumpMinorPreMajor is ${element.bumpMinorPreMajor}`, async () => {
                ...
                chai_1.expect(openPROptions).to.have.property('includePackageName').to.be
                    .false;
                ...
                chai_1.expect(openPROptions)
                    ...
                    .to.eql([
                    new package_lock_json_1.PackageLockJson({
                    ...
                    new samples_package_json_1.SamplesPackageJson({
                    ...
                    new package_json_1.PackageJson({
                        path: 'package.json',
                        changelogEntry: perUpdateChangelog,
                        version: expectedVersion,
                        packageName: pkgName,
                        contents: utils_1.buildGitHubFileContent('./test/releasers/fixtures/node', 'package.json'),
                    }),
                ]);
            });
        });
    ...
    mocha_1.describe('run', () => {
        mocha_1.it('creates a release PR without package-lock.json', async function () {
            const github = new github_1.GitHub({ owner: 'googleapis', repo: 'node-test-repo' });
            const mock = mockGithub({
                github,
                fixtures: ['package.json'],
        ...
        mocha_1.it('creates a release PR with package-lock.json', async function () {
            const github = new github_1.GitHub({ owner: 'googleapis', repo: 'node-test-repo' });
            const mock = mockGithub({
                github,
        ...
        mocha_1.it('creates a release PR with npm-shrinkwrap.json', async function () {
            const github = new github_1.GitHub({ owner: 'googleapis', repo: 'node-test-repo' });
            const mock = mockGithub({
                github,
        ...
        mocha_1.it('creates release PR relative to a path', async function () {
            const github = new github_1.GitHub({ owner: 'googleapis', repo: 'node-test-repo' });
            const mock = mockGithub({
                github,
            ...
            const releasePR = new node_1.Node({ github, path: 'packages/foo' });
            helpers_1.stubSuggesterWithSnapshot(sandbox, this.test.fullTitle());
            await releasePR.run();
            mock.verify();
        });
        mocha_1.it('does not support snapshot releases', async () => {
            const github = new github_1.GitHub({ owner: 'googleapis', repo: 'node-test-repo' });
            const mock = mockGithub({ github, fixtures: [], notFound: [] });
            const releasePR = new node_1.Node({ github, snapshot: true });
            const pr = await releasePR.run();
            mock.verify();
            assert.strictEqual(pr, undefined);
        });
        mocha_1.it('uses detected package name in branch', async function () {
            const github = new github_1.GitHub({ owner: 'googleapis', repo: 'node-test-repo' });
            const mock = mockGithub({
                github,
                fixtures: ['package.json'],
            ...
            const releasePR = new node_1.Node({ github, monorepoTags: true });
            helpers_1.stubSuggesterWithSnapshot(sandbox, this.test.fullTitle());
            const pr = await releasePR.run();
            mock.verify();
            assert.strictEqual(pr, 22);
    ...
    mocha_1.describe('getPackageName', () => {
        for (const packageName of ['@over/write-me', undefined]) {
            mocha_1.it('finds package name in package.json using ' +
                `packageName input option: ${packageName}`, async () => {
                const github = new github_1.GitHub({
                    owner: 'googleapis',
                    repo: 'node-test-repo',
                });
                ...
                mock
                    ...
                    .once()
                    .resolves(utils_1.buildGitHubFileRaw('{"name":"@google-cloud/pkg-name"}'));
                const node = new node_1.Node({ github, packageName });
                const expectedPackageName = await node.getPackageName();
                chai_1.expect(expectedPackageName.name).to.equal('@google-cloud/pkg-name');
                chai_1.expect(expectedPackageName.getComponent()).to.equal('pkg-name');
            });
        }
    });
});
